...
function locales(request) {
  ...
  const shouldHandleLocale = !/^\/(docs|realgrid|realchart|realpivot|offline|js|styles|data|api|_next)\//.test(nextUrl.pathname) && !/\.(jpe?g|svg|png|webmanifest|xml|ico|txt|mp4)$/.test(nextUrl.pathname) && nextUrl.locale !== "" && // not Server-Side Error page
